...
<div>
    ...
    <flux:card class="mt-6 overflow-hidden p-0">
        ...
        @if (count($selected) > 0)
            <div
                ...
                <flux:dropdown>
                    <flux:button size="sm" icon-trailing="chevron-down">Set status</flux:button>
                    <flux:menu>
                        @foreach (CategoryStatus::cases() as $s)
                            <flux:menu.item wire:click="bulkSetStatus('{{ $s->value }}')">{{ $s->label() }}
                            </flux:menu.item>
                        @endforeach
                    </flux:menu>
                </flux:dropdown>

                <flux:button size="sm" icon="trash-2" wire:click="bulkDelete"
                    class="text-red-500! hover:text-red-600!">Delete</flux:button>

                <flux:spacer />

                <flux:button size="sm" wire:click="clearSelection">Clear</flux:button>
            </div>
        @endif
                    ...
                    <flux:table.row :key="$category->id">
                        ...
                        <flux:table.cell variant="strong">
                            <div class="flex items-center gap-3">
                                ...
                                @else
                                    <div
                                        class="flex size-10 shrink-0 items-center justify-center rounded-md bg-zinc-100 dark:bg-zinc-800">
                            ...
                            <flux:dropdown align="end">
                                ...
                                <flux:menu>
                                    ...
                                    <flux:menu.item icon="clock" icon-variant="outline"
                                        :href="route('admin.activity.item', ['category', $category->id])" wire:navigate>
                                        Activity log
                                    </flux:menu.item>
                                    <flux:menu.separator />
    ...
    <flux:modal name="bulk-delete-categories" class="max-w-sm">
        ...
        <flux:subheading class="mt-1">Delete <strong>{{ count($selected) }}</strong> selected
            {{ str('category')->plural(count($selected)) }}? Categories with products attached will be skipped.
        </flux:subheading>
    ...
    <flux:modal name="delete-category" class="max-w-sm">
        ...
        <flux:subheading class="mt-1">Are you sure you want to delete <strong>{{ $deletingName }}</strong>? This
            cannot be undone.</flux:subheading>
        <div class="mt-6 flex justify-end gap-3">
            <flux:modal.close>
                <flux:button variant="ghost">Cancel</flux:button>
            </flux:modal.close>
