...
new #[Title('Add Shipping Method')] class extends Component {
    ...
    public function save(): void
    {
        try {
            $this->form->store();
            $this->dispatch('notify', title: 'Method Added', variant: 'success', message: 'Shipping method created successfully.');
            $this->redirectRoute('admin.logistics.configuration.methods.index', navigate: true);
        } catch (\Illuminate\Validation\ValidationException $e) {
            throw $e;
        } catch (\Throwable $e) {
            logger()->error('Failed to create shipping method.', ['exception' => $e->getMessage()]);
            $this->dispatch('notify', title: 'Save Failed', variant: 'danger', message: 'Something went wrong. Please try again.');
        }
    }
}; ?>

<x-admin.logistics.layout heading="Add Shipping Method"
    subheading="Configure a new delivery option for customers at checkout.">
    ...
    @push('breadcrumbs')
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Add Method</flux:breadcrumbs.item>
        </flux:breadcrumbs>
    @endpush
