...
<ui-modal {{ $attributes }} data-flux-modal class="fixed z-100">
    <?php if ($trigger): ?>
    {{ $trigger }}
    <?php endif; ?>

    <dialog wire:ignore.self {{-- This needs to be here because the dialog element adds a "close" attribute that isn't durable... --}} {{ $dialogAttributes->class($classes) }} <?php if ($name): ?>
        data-modal="{{ $name }}" <?php endif; ?> <?php if ($flyout): ?> data-flux-flyout <?php endif; ?>
        <?php if ($overflow): ?> data-flux-modal-overflow <?php endif; ?> @unblaze(scope: ['name' => $name])
        x-data="fluxModal(@js($scope['name']), @js(isset($__livewire) ? $__livewire->getId() : null))" @endunblaze x-on:modal-show.document="handleShow($event)"
        ...
        <div class="flex min-h-full items-center justify-center p-4 sm:p-6">
            <div {{ $contentAttributes->class($contentClasses) }} data-flux-modal-content>
                ...
                <div class="absolute top-0 end-0 mt-4 me-4">
                    <flux:modal.close>
                        <flux:button variant="ghost" size="sm" aria-label="Close modal"
                            class="text-zinc-400! hover:text-zinc-800! dark:text-zinc-500! dark:hover:text-white! group cursor-pointer">
                            <x-slot name="icon">
                                <flux:icon.x-mark
                                    class="size-5 group-hover:rotate-45 transition-all duration-150 ease-in-out" />
                            </x-slot>
                        </flux:button>
                    </flux:modal.close>
                </div>
        ...
        <div class="absolute top-0 end-0 mt-4 me-4">
            <flux:modal.close>
                <flux:button variant="ghost" size="sm" aria-label="Close modal"
                    ...
                    <x-slot name="icon">
                        <flux:icon.x-mark
                            class="size-5 group-hover:rotate-90 transition-all duration-150 ease-in-out" />
                    </x-slot>
                </flux:button>
            </flux:modal.close>
        </div>
