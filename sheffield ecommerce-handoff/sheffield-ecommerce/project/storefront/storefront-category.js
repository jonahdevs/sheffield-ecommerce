// Sheffield Category page — scoped catalog + filters (faithful to category.blade.php)
(function () {
  const SF = window.SF, C = window.SFChrome, svg = C.svg;
  const ABS_MIN = 0, ABS_MAX = 6000000, STEP = 50000;
  const fmtK = v => 'KES ' + Number(v).toLocaleString();

  C.mount('shop');

  // Resolve category from ?c=
  const param = new URLSearchParams(location.search).get('c');
  const cat = SF.categories.find(c => c.name === param) || SF.categories[0];
  document.title = cat.name + ' — Sheffield';
  document.getElementById('bcName').textContent = cat.name;
  document.getElementById('catTitle').textContent = cat.name;
  document.getElementById('catDesc').textContent = cat.desc || '';

  // Scope catalog to this category
  const pool = SF.catalog.filter(p => p.cat === cat.name);
  const brands = [...new Set(pool.map(p => p.brand))].sort();

  const S = { brands: new Set(), pmin: ABS_MIN, pmax: ABS_MAX, rating: 0, stock: false, sort: 'popularity' };
  const hasFilters = () => S.brands.size || S.stock || S.rating>0 || S.pmin>ABS_MIN || S.pmax<ABS_MAX;
  ...
  function section(title, open, body) {
    return `<div class="filter-section px-5 py-4" data-open="${open?'true':'false'}">
      ...
      </button><div class="filter-body mt-3">${body}</div></div>`;
  }
  const checkRow = (f,v,l) => `<label class="flex cursor-pointer items-center gap-2.5 text-[13.5px] text-ink-2"><input type="checkbox" data-f="${f}" value="${v}" class="size-4 rounded border-zinc-300 text-brand-500 focus:ring-brand-500"/><span class="flex-1">${l}</span></label>`;
  function priceBody() {
    return `<div class="price-filter">
      ...
      <div class="mt-4 flex items-end gap-2">
        <div class="flex-1"><label class="mb-1 block text-[11px] text-ink-4">Min</label><input type="number" data-f="pminN" class="w-full rounded border border-zinc-300 px-2 py-1.5 text-[13px] tabular-nums focus:border-brand-500 focus:outline-none"/></div>
        <span class="pb-2 text-ink-4">—</span>
        <div class="flex-1"><label class="mb-1 block text-[11px] text-ink-4">Max</label><input type="number" data-f="pmaxN" class="w-full rounded border border-zinc-300 px-2 py-1.5 text-[13px] tabular-nums focus:border-brand-500 focus:outline-none"/></div>
      </div></div>`;
  }
  function ratingBody() {
    let h = '<div class="flex flex-col gap-2.5">';
    for (let r=4;r>=1;r--){ let s=''; for(let i=1;i<=5;i++) s+=svg('star','size-4 '+(i<=r?'text-amber-500':'text-zinc-300')); h+=`<label class="flex cursor-pointer items-center gap-2"><input type="radio" name="rating" data-f="rating" value="${r}" class="size-4 border-zinc-300 text-brand-500 focus:ring-brand-500"/><span class="flex items-center gap-1.5">${s}<span class="ms-1 text-[12.5px]">&amp; up</span></span></label>`; }
    return h+'</div>';
  }
  function panelHTML() {
    ...
    return `<div class="divide-y divide-zinc-200 rounded-md border border-zinc-200 bg-white text-sm">
      ...
      ${brands.length?section('Brand',false,`<div class="flex flex-col gap-2">${brandRows}</div>${brandMore}`):''}
      ${section('Availability',true,checkRow('stock','1','In stock — ships now'))}
    </div>`;
  }

  function wirePanel(root) {
    root.querySelectorAll('.filter-toggle').forEach(b => b.onclick = () => { const s=b.closest('.filter-section'); s.dataset.open = s.dataset.open==='true'?'false':'true'; });
    ...
    const pmin = root.querySelector('[data-f="pmin"]'), pmax = root.querySelector('[data-f="pmax"]'), pminN = root.querySelector('[data-f="pminN"]'), pmaxN = root.querySelector('[data-f="pmaxN"]');
    if (pmin) {
    ...
    if (more) more.onclick = () => { const hid=root.querySelector('.brand-extra.hidden'); root.querySelectorAll('.brand-extra').forEach(e=>e.classList.toggle('hidden',!hid)); more.textContent = hid?'Show fewer':`Show all ${brands.length} brands`; };
  }
  function syncPrice() {
    document.querySelectorAll('.price-filter').forEach(pf => {
      pf.querySelector('[data-f="pmin"]').value=S.pmin; pf.querySelector('[data-f="pmax"]').value=S.pmax;
      pf.querySelector('[data-f="pminN"]').value=S.pmin>ABS_MIN?S.pmin:''; pf.querySelector('[data-f="pmaxN"]').value=S.pmax<ABS_MAX?S.pmax:'';
      pf.querySelector('.pf-min-read').textContent=fmtK(S.pmin); pf.querySelector('.pf-max-read').textContent=fmtK(S.pmax);
      const fill=pf.querySelector('.pf-fill'); fill.style.left=((S.pmin-ABS_MIN)/(ABS_MAX-ABS_MIN)*100)+'%'; fill.style.right=(100-(S.pmax-ABS_MIN)/(ABS_MAX-ABS_MIN)*100)+'%';
    });
  }
  ...
  function filtered() {
    let list = pool.filter(p => {
      ...
      const pr = p.sale ?? p.price;
      if (pr==null) return !(S.pmin>ABS_MIN);
  ...
  function chips() {
    ...
    const chip=(l,on)=>`<button class="sf-chip inline-flex h-7 cursor-pointer items-center gap-1.5 rounded-full bg-surface-sunken px-3 text-[12.5px] font-medium text-ink-2 hover:bg-zinc-200" data-clear="${on}">${l} ${svg('x','size-3 text-ink-3')}</button>`;
    ...
    el.querySelectorAll('.sf-chip').forEach(b => b.onclick = () => {
      ...
      if (v.startsWith('brand:')) S.brands.delete(v.slice(6));
      else if (v==='stock') S.stock=false; else if (v==='price'){S.pmin=ABS_MIN;S.pmax=ABS_MAX;} else if (v==='rating') S.rating=0;
      applyAll();
    });
  }
  function applyAll() {
    ...
    document.getElementById('clearInline').classList.toggle('hidden', !hasFilters());
    document.getElementById('mfDot').classList.toggle('hidden', !hasFilters());
    chips(); syncControls();
  }
  function clearAll() { S.brands.clear(); S.pmin=ABS_MIN; S.pmax=ABS_MAX; S.rating=0; S.stock=false; applyAll(); }

  document.getElementById('filters').innerHTML = panelHTML();
  document.getElementById('filtersMobile').innerHTML = panelHTML();
  ...
  document.getElementById('mobileFilterBtn').onclick = () => drawer.classList.remove('hidden');
  document.getElementById('drawerClose').onclick = () => drawer.classList.add('hidden');
  document.getElementById('drawerBackdrop').onclick = () => drawer.classList.add('hidden');
  document.getElementById('drawerApply').onclick = () => drawer.classList.add('hidden');
