...
## 7. Image sourcing (July 2026) — downloaded to `Downloads/hatton-images/`

**24 files.** Two sources: the official `hatton-tech.com` gallery (served from
`ijrorwxhlqnrln5p-static.micyjz.com`, clean studio renders) and the MIC storefront
(`image.made-in-china.com`, shop-floor and installed-site photos). Pulled straight via
`curl` with a browser User-Agent and a matching `Referer`; no auth needed.
...
that have nothing to do with a hood dishwasher.

### IMG/DWW/00151 — HT-T2 under-counter (12 files)

| File | Source | Content |
|---|---|---|
| `IMG-DWW-00151__HT-T2-official-01..06.jpg` | hatton-tech.com (94 KB–272 KB) | Studio renders of the **blue-fascia / rotary-knob** generation. `-01` is the hero shot — door open, rack of plates loaded, "HATTON / UNDER COUNTER" badge clearly legible. **Best storefront candidate of the whole set.** |
| `IMG-DWW-00151__HT-T2-mic-01..06.webp` | MIC HT-T2 listing (4 KB–25 KB) | Shop-floor photos of the **black digital-touch-panel** generation (see `-05`, door open on the wash chamber). Lower resolution; useful mainly as evidence for the generation split in §3.1. `-05` also shows a shop note reading "380V". |

### IMG/DWW/00149 — HT-Z1 hood type (12 files) ⚠ mostly wrong machine

| File | Content | Usable? |
|---|---|---|
| `IMG-DWW-00149__HT-Z1-alt-04.webp` | Clean installed **hood-type** Hatton with hood down, embossed HATTON badge, blue nameplate, rack table alongside | ✅ **best HT-Z1 photo** |
| `IMG-DWW-00149__HT-Z1-alt-06.webp` | Hood-type Hatton still in blue protective film, hood down, inlet plumbing visible | ✅ genuine, but the film reads as "unfinished" |
| `IMG-DWW-00149__HT-Z1-mic-06.webp` | Hood-type in service, **hood raised**, blue rack of plates loaded — shows the 505 mm entrance and rack clearance well | ✅ genuine, in-situ |
| `IMG-DWW-00149__HT-Z1-mic-01/02/03/04/05.webp` | **Rack-conveyor and flight-type machines** — long tunnel cabinets, drying sections, curtain strips | ❌ wrong product |
| `IMG-DWW-00149__HT-Z1-alt-01/02/03/05.webp` | Same — flight-type conveyor installs, one still in factory film | ❌ wrong product |

Only **3 of 12** HT-Z1 images are actually a hood-type dishwasher. The other 9 were kept
rather than deleted so the mismatch is documented, but they must not be attached to
`IMG/DWW/00149`.

### Why this matters for the existing images
...
`under-counter-dishwasher-hattons-imgdww00151.png` is **24 KB**. The official HT-T2 renders
downloaded here are 94–272 KB at studio quality — a clear upgrade for 00151. For 00149 the
best available is `alt-04` at 34 KB, which is better but still not a studio render; Hatton
does not appear to publish one for the hood machine.
...
5. **Upgrade IMG/DWW/00151's product image** to the official 1000 px render (§7).
