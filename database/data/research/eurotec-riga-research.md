...
**6 files** (was 7 — see §10.1), named by SKU for manual review, same workflow as the
Brema/Santos passes.

| File | Pixels / size | Source | Notes |
|---|---|---|---|
| `IMG-BUF-00012__alrazana-RIB-3520EB-1.jpg` | 1000×1000, 35 KB | https://razana-media-992382793141.s3.ap-south-1.amazonaws.com/uploads/2024/01/RIB-3520EB.jpg | **Best candidate.** White-background 3/4 render showing hob + detached control pod + lead |
| `IMG-BUF-00012__alrazana-RIB-3520EB-2.jpg` | 1000×1000, 14 KB | https://razana-media-992382793141.s3.ap-south-1.amazonaws.com/uploads/2024/01/RIB-3520EB-2.jpg | Second angle, hob body only (no control pod visible) |
| `IMG-BUF-00012__official-rigainternational-slider-TOOSMALL.webp` | **300×200, 3 KB** | https://web.archive.org/web/20240609185926id_/https://rigainternational.com/images/slider-home/rib-3520eb.webp | **Only official-domain asset in existence.** Correct product, but a homepage slider thumbnail — proven cap (§10.1). Kept purely as provenance: it is the same photograph as the Alrazana pair, which is what proves those are the manufacturer's own |
| `IMG-BUF-00012__redfox-careho-00025504-TOOSMALL.png` | **300×200, 27 KB** | https://careho.pl/pubimg/careho-00025504.png | ⚠ **Heavily watermarked** ("careho.pl" tiled across it) — reference only, unusable on the storefront. Proven cap (§10.1) |
| `IMG-BUF-00012__successor-official-RIA-3521EB.png` | 1080×1080, 328 KB | https://www.eurotecriga.com/wp-content/uploads/2025/07/RIA-3521EB.png | ⚠ **Successor model, not ours** (§7). Square-canvas official render; cosmetically near-identical. Use only if the supplier has switched to the RIA generation |
| `IMG-BUF-00012__successor-official-RIA-3521EB-2.png` | **4781×3352, 6.0 MB** | https://www.eurotecriga.com/wp-content/uploads/2025/06/RIA-3521EB.png | ⚠ Successor model. **Print-resolution native original** of the same render — upgraded from 500×500 in the re-sourcing pass (§10.1) |
...
### 10.1 Re-sourcing pass (July 2026) — one real upgrade, two proven caps, one deletion

A minimum-resolution rule (**800 px long edge, 1000 px+ preferred**) was introduced after
this brand's original image pass. Four of the seven files were below it. This is the hardest
brand in the catalogue for imagery — the model is discontinued, the official product image
was never archived, and the only official-domain asset is a 300 px slider — so each file was
probed to a definite ceiling rather than forced.

**Upgraded (1):**

| File | Before | After |
|---|---|---|
| `IMG-BUF-00012__successor-official-RIA-3521EB-2.png` | 500 × 500, 38 KB | **4781 × 3352, 6.0 MB** |

Found via the **WordPress REST API**, which is also what proves the old file was not simply
a resize: `GET /wp-json/wp/v2/media?search=RIA-3521EB` reports `RIA-3521EB-2.png` as
`full` = **500 × 500** — that attachment is genuinely 500 px and no suffix-stripping would
have helped it. The same query exposes *separate* attachments of the identical render at
1920 × 1080, and at 2560 × 1795 under WordPress's `-scaled` name. `-scaled` means WordPress
downsized a larger upload, so the pre-scale original was requested directly at
`/wp-content/uploads/2025/06/RIA-3521EB.png` → **4781 × 3352**.

**Not an upscale**, checked per the second-batch warning: a 1:1 crop of the 4781 px file
shows crisp anti-aliased hairlines on the hob's dial ring and clean brushed-steel grain.
A Lanczos enlargement of the 500 px or 1080 px copy could not carry that detail. It is also
larger than WordPress's own 2560 px `-scaled` derivative, i.e. it is upstream of every
published size, which is the opposite of an upscale. No `-UPSCALED` / `-NATIVE` split needed.

**Deleted (1):**

`IMG-BUF-00012__legacy-sheffieldafrica-RIB35EB.jpg` (450 × 450, 14 KB) — opened side by side
with `IMG-BUF-00012__alrazana-RIB-3520EB-1.jpg` and confirmed to be **the same photograph**:
identical 3/4 angle, identical control pod placement, identical cable run. The legacy file is
that shot letterboxed into a 450 px square. The Alrazana 1000 × 1000 supersedes it outright,
so it was removed. (Both `sheffieldafrica.com/public/storage/uploads/…` and
`…/storage/uploads/…` return the same 15 358-byte file; an `/original/` path segment 404s.)

**Proven capped (2), kept and suffixed `-TOOSMALL`:**

| File | Ceiling | What was probed |
|---|---|---|
| `…__official-rigainternational-slider-TOOSMALL.webp` | **300 × 200** | Wayback CDX for `rigainternational.com*` filtered on `3520` returns **exactly one capture** of one URL — `/images/slider-home/rib-3520eb.webp`, 3 950 B, 2024-06-09. There is no second capture and no larger sibling. CDX for `eurotecriga.com/images*` (300 rows) confirms the model's own product image `…/images/prodotti/piastre-ad-induzione/RIB-3520EB.jpg` **was never captured at all** — only unrelated `foto-slide-home/` microwave assets. The live `rigainternational.com` now serves `eurotecriga.com` WordPress assets and carries **no RIB-3520EB image whatsoever** (only MW\* microwaves and the current RIA-\* hobs) |
| `…__redfox-careho-00025504-TOOSMALL.png` | **300 × 200** | `careho.pl` serves only two variants of this asset: `/pubimg/careho-00025504.png` (300 × 200, the master) and `/pubimg/t240x240/careho-00025504.png` (the thumbnail). `_big`, `/big/`, `/orig/` and `/img/` path variants all 404. The product page exposes no `srcset`, `data-zoom-image` or `data-large_image`. Watermarked and reference-only regardless |

**Also probed and ruled out:**

- **The live Eurotec Riga site has no RIB-3520EB asset.** WP REST media searches for
  `RIB-3520`, `RIB`, `3520` and `induction` all return **zero** results; `RIA-35` returns 28
  assets, all successor-generation. The discontinued model was never migrated to WordPress —
  consistent with §7.
- **The 2018 price-list PDF was opened with PyMuPDF and all 14 embedded image objects
  extracted losslessly.** The largest are **376 × 175** and **315 × 123** product
  thumbnails — an order of magnitude below the bar. This was the most promising remaining
  lead (it is the one archived document that pictures the RIB-3520EB on manufacturer
  letterhead) and it is a dead end for imagery.

**Net: 7 files → 6.** Two remain below the bar, both now `-TOOSMALL` and both reference-only.
**The SKU is nonetheless fully covered**: the Alrazana pair at 1000 × 1000 clear the bar and
are almost certainly Eurotec Riga's own photography — the 300 px official slider, small as it
is, is the evidence for that, which is why it was kept rather than deleted. **Do not re-run
the Wayback search** — the official RIB-3520EB product image does not exist in any public
archive.
