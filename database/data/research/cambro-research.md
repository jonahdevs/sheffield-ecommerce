...
Supersedes the archived research below.
...
⚠ **This file REVERSES the code recommendation made in the archived research below.** The earlier
...
The archived research below concluded **add-on** because our record says *"Designed to share posts,
...
**Re-sourcing pass, July 2026 — all 15 files are now ≥ 800 px on the long edge.** Six files
were staged before the minimum-resolution rule existed and have been resolved: four upgraded
in place, two deleted as superseded. Two brand-new official Cambro files were added. The two
lessons that made it work are worth keeping:

1. **WebstaurantStore has an undocumented `xxl` rendition.** The path segment is a size name,
   not a fixed number: `.../images/products/{thumbnails|small|medium|large|extra_large|xxl}/<dir>/<id>.jpg`.
   `original`, `zoom`, `full`, `xl` and `huge` all 404. `xxl` is **native-capped, never
   upscaled** — proof: three of these assets return 2000 × 2000 from `xxl` while a fourth
   returns 1050 × 1050, i.e. the CDN serves the true source size rather than padding
   everything to a ceiling. FFT radial-energy checks against Lanczos controls confirmed
   genuine detail in every one (0.06–0.09 relative energy in the top band vs 0.02 for a
   synthetic upscale).
2. **Widen's `original` rendition returns the native master.** Swap the format segment for
   `original` — `cambro.widen.net/content/<hash>/original/<FILENAME>.webp` — and the DAM
   303-redirects to a signed `cf-store.widencdn.net` URL holding the full-size JPEG. The
   sized renditions (`webp`, `jpeg`, `png`) all cap at 2048 px regardless of any `?w=` value,
   which is silently ignored; `original` on the same asset yields 4104 × 2988. `tiff` 404s.

#### IMG/STO/00001 — Camshelving Basics Plus

| File | px | Size | Source | Note |
|---|---|---|---|---|
| `IMG-STO-00001__CBU213672V4580-webstaurant.jpg` | **2000 × 2000** | 191 KB | https://cdnimg.webstaurantstore.com/images/products/xxl/378104/3027656.jpg | **Best candidate.** The only render found of the **exact** 21 × 36 × 72 vented 4-shelf Brushed Graphite unit. Clean white background. ⬆ upgraded from the 600 px `large` rendition |
| `IMG-STO-00001__CBA183672V4580-addon-webstaurant.jpg` | **2000 × 2000** | 158 KB | https://cdnimg.webstaurantstore.com/images/products/xxl/378064/3019652.jpg | The **add-on** equivalent — use this one instead if §3.1 resolves to `CBA`. ⬆ upgraded from 600 px |
| `IMG-STO-00001__CBU-basics-plus-starter-render.webp` | 985 × 1427 | 127 KB | https://cambro.widen.net/content/ugkb8vixxz/webp/CBU183064V4580_A1R0_0117_s02.webp | Official Cambro render, but of `CBU183064` (18 × 30 × 64) — same series/finish, **different size** |
| `IMG-STO-00001__CBU-basics-plus-starter-render2.webp` | 797 × 1500 | 129 KB | https://cambro.widen.net/content/mpafeknh5o/webp/CBU183064V4580_A1R0_0918_S01.webp | as above |
| `IMG-STO-00001__CBA-basics-plus-addon-render.webp` | 1071 × 1500 | 82 KB | https://cambro.widen.net/content/gchlzsxil2/webp/CBA183064V4580_A1R0_0918_S01.webp | Official add-on render, `CBA183064` |
| `IMG-STO-00001__CBA-basics-plus-addon-lifestyle.webp` | 1326 × 1500 | 265 KB | https://cambro.widen.net/content/fmrcbid1lz/webp/CBA183064V4580_A1LK_0616_s02.webp | In-kitchen lifestyle shot |
| `IMG-STO-00001__CBA-basics-plus-addon-detail.webp` | 1500 × 1095 | 340 KB | https://cambro.widen.net/content/e4duvs5cdx/webp/CBA183064V4580_B1RK_1013_s05.webp | Detail/loaded shot |
...
**This caveat is unchanged by the re-sourcing pass, and no wrong-size render was swapped in
for a size-accurate one.** All three Basics Plus pages (`/`, starter, add-on) were re-scraped
for `CB*` Widen assets: the complete set is `CBU183064V4580`, `CBU183064S4580`,
`CBA183064V4580`, `CBA183064S4580`, `CBSK1830V1580`, `CBSK1830S1580`, `CBUPR1826S6580`,
`CBPK1864580`, `CBDS18H6580`, `CBCC1580`, `CBID6000` and `CBR1830151` — **no `CBU213672` or
`CBA183672` render exists anywhere on cambro.com.** The size-accurate WebstaurantStore files
are now 2000 px, so the size-accurate option is also the highest-resolution option and the
trade-off that used to exist here is gone.
...
| File | px | Size | Source | Note |
|---|---|---|---|---|
| `IMG-DWW-00107__PR59314151-official-loaded-pans.jpg` | **4104 × 2988** | 4.1 MB | https://cambro.widen.net/content/77ljlraxqj/original/PR59314151_A1LL_0119_S04.webp | 🆕 **Official Cambro render of the exact 5 × 9 part.** Rack loaded with amber food pans, white background. Highest-resolution file in the whole Cambro set |
| `IMG-DWW-00107__PR59314151-official-loaded-plates.jpg` | **2749 × 1621** | 1.6 MB | https://cambro.widen.net/content/jt1yfgkf2u/original/PR59314151_A1CL_0818_s06.webp | 🆕 Official, exact part, two racks loaded with plates |
| `IMG-DWW-00107__PR59314-webstaurant-1.jpg` | **2000 × 2000** | 512 KB | https://cdnimg.webstaurantstore.com/images/products/xxl/59571/2672179.jpg | **Best storefront candidate** — in-use shot, plates being loaded. ⬆ upgraded from 600 × 600 |
| `IMG-DWW-00107__PR59314-webstaurant-2.jpg` | **1050 × 1050** | 167 KB | https://cdnimg.webstaurantstore.com/images/products/xxl/59571/3002600.jpg | Bare rack render on white. ⬆ upgraded from 600 × 600; 1050 px is this asset's native cap (`xxl` returned 2000 px for its siblings) |
| `IMG-DWW-00107__PR314151-official-front.webp` | 1643 × 1316 | 320 KB | https://cambro.widen.net/content/ancrloehtx/webp/PR314151_A1R0_0818_S03.webp | Official Cambro, but the **9 × 9** `PR314` — see caution below |
| `IMG-DWW-00107__PR314151-official-closeup.webp` | 1201 × 811 | 93 KB | https://cambro.widen.net/content/3iajw5cu4p/webp/PR314151_A1C0_0818_s01.webp | Official; this is the image the 8 × 8 peg count in §3.2 was read off |
| `IMG-DWW-00107__PR314151-official-loaded.webp` | 994 × 980 | 156 KB | https://cambro.widen.net/content/n0gtizj8gw/webp/PR314151_A1RL_1016_S02.webp | Official, loaded with plates |
| `IMG-DWW-00107__PR314151-official-lifestyle.webp` | 1800 × 1560 | 281 KB | https://cambro.widen.net/content/fmonxrqjvq/webp/PR314151_A1LK_0818_s04.webp | Official lifestyle |

**Deleted in the re-sourcing pass** (both superseded by a verified larger original of the
same photograph, both opened and compared side by side first):

| Deleted file | Was | Superseded by |
|---|---|---|
| `IMG-DWW-00107__PR59314151-ckitchen-1.webp` | 500 × 500, 11 KB | `PR59314151-official-loaded-pans.jpg` — **identical shot** (same rack, same amber pans, same angle), 4104 × 2988 straight from Cambro's own DAM |
| `IMG-DWW-00107__PR59314151-ckitchen-2.webp` | 500 × 500, 15 KB | `PR59314-webstaurant-2.jpg` — **identical shot** (bare grey rack, same 3/4 angle), now 1050 × 1050 |

ckitchen was probed for a larger copy before deleting: `cdn.ckitchen.com/pmidimages/<slug>.webp`
is the only path that resolves (`images`, `pmimages`, `bigimages`, `originalimages`,
`product-images` all 403; `.jpg`/`.png` 403), and `?w=`, `?width=`, `?size=full` are all
ignored — the CDN returns the same 11 KB body. ckitchen is capped at 500 px.

⚠ **Peg-layout caution — REVISED.** The original pass concluded that Cambro's Peg and Tray
Racks page only carried renders of `PR314151`, the **9 × 9** rack. That was wrong: the page
also carries **two official `PR59314151` assets** — `PR59314151_A1LL_0119_S04` and
`PR59314151_A1CL_0818_s06` — which the first sweep missed. Both are now downloaded at native
resolution and both were visually confirmed as the **5 × 9** configuration.

So the trade-off described before no longer applies. The correct framing now:

- **Exact 5 × 9 `PR59314151`, official Cambro, print resolution:** the two new
  `official-loaded-*` files. These are the best available in every respect.
- **Exact `PR59314`, reseller, clean product shots:** the two WebstaurantStore files
  (2000 × 2000 and 1050 × 1050).
- **`PR314151` — still the 9 × 9 rack, still the wrong configuration.** The four
  `PR314151-official-*` files are retained for reference only. Same moulding family, same
  tray, same colour, but the peg spacing visibly differs on one half. **Do not put one of
  these on the storefront** now that exact-part renders exist at higher resolution.

A site-wide search of cambro.com returns only those two `PR59314151` assets, so that is the
ceiling on official 5 × 9 photography.
