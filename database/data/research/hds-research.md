...
The previous pass is the archived research below; several of its
...
5. **SKU typo — FIXED 2026-07-30:** `IMG/HOT/OO438` (Gas Burners with Oven and 24" Griddle/Salamander)
   used the letter `O` twice instead of the digit `0`. Corrected to `IMG/HOT/00438`, and the five
   image files were renamed on disk to match. Not a guess: the SAP export (`img-hot.xlsx`) carries
   `IMG/HOT/00438` — "GAS BURNERS WITH OVEN AND 24" GRIDDLE/SALAMANDER", Make HDS — directly after
   `IMG/HOT/00437` (6 Burner Gas Range HDSGR-36), and no SAP item code in that workbook contains a
   letter-O at all. See `sap-reconciliation-research.md` §"The 17 SKUs absent from SAP".
...
| IMG/HOT/00438 | Gas Burners with Oven and 24" Griddle/Salamander | (blank) | https://heavydutysystems.com/product/gas-burners-with-oven-and-24-griddle/ | same page | N/A — archived/incomplete record; SKU typo fixed, see §2 trap 5 |
...
- §2 trap 5 (`IMG/HOT/OO438` SKU typo — letter-O instead of zero) — resolved as a data-entry fix
  on 2026-07-30, not something research itself could confirm.
