...
| File | Source | Px / size | Notes |
|---|---|---|---|
| `IMG-OVE-00107__CTB-SGL-front.jpg` | https://www.katom.com/015-CTBSINGLE2083.html | **1600 × 1600**, 126 KB | **Recommended primary.** Clean white-background 3/4 front, on the 4" legs, dial control panel legible. Best available shot of this SKU. |
| `IMG-OVE-00107__CTB-SGL-on-stand-alt.webp` | `https://cdn.ckitchen.com/pimages/blodgett-ctb-sgl-deck-electric-convection-oven-with-contols-208v-60-1-ph-5-6-kw-27-0-amps-direct-20220324160531261.webp` | **1000 × 1000**, 32 KB | On-stand composition on the optional caster stand, empty pans. Correct CTB — left hinge, controls right. **Upgraded from 600 × 600 in the July 2026 re-sourcing pass (§8.1).** |
| `IMG-OVE-00107__CTB-SGL-on-stand-official-TOOSMALL.jpg` | https://www.blodgett.com/wp-content/uploads/2021/02/CTB.jpg | ⚠ **400 × 595**, 64 KB | Official Blodgett render of the *food-styled* on-stand composition (loaded racks and pans) — a genuinely different shot from the `-alt` file above, not a crop of it. Correct CTB. **Proven capped below the bar — see §8.1.** |
| `IMG-OVE-00107__CTB-SGL-lifestyle-1.jpg` | katom | 1600 × 1600, 182 KB | Kitchen lifestyle shot. Usable as a gallery image; control-panel legends are illegible. |
| `IMG-OVE-00107__CTB-SGL-lifestyle-2.jpg` | katom | 1600 × 1600, 193 KB | ⚠ **Looks synthetic/AI-composited** — the BLODGETT badge is distorted and the panel legends are nonsense. Use only if a lifestyle filler is wanted, and preferably not at all. |
| `IMG-OVE-00107__CTBR-on-stand-official.jpg` | https://www.blodgett.com/wp-content/uploads/2024/11/CTBR-onStand.jpg | 2259 × 3503, 933 KB | ⚠ **WRONG VARIANT — do not publish on this SKU.** Highest-resolution official image on blodgett.com, but it is the **CTBR**: door hinged on the **right**, controls on the **left** — the mirror image of what we sell. Kept only as a construction/detail reference. |
...
### 8.1 Re-sourcing pass (July 2026) — one upgraded, one proved capped

Both on-stand files were below the 800 px minimum-long-edge rule. **They are two different
photographs**, not two crops of one, so they had to be chased separately: the `-official`
one has food loaded in the racks and lower pans, the `-alt` one is the same staging with
empty stainless pans.

**Upgraded — `-on-stand-alt`, 600 × 600 → 1000 × 1000.** ckitchen serves the same asset from
three CDN buckets under one filename; only the bucket segment changes:
`pthumbs/` → 500², `pmidimages/` → 600² (what was originally taken), **`pimages/` → 1000²**.
`pbigimages`, `plargeimages`, `pfullimages`, `poriginals` and `pzoom` all 403, so 1000 × 1000
is ckitchen's ceiling. Verified by eye as the identical render before the 600 px file was
deleted, and confirmed natively sharp (round-trip resampling shows no RMSE collapse).

**Capped — `-on-stand-official`, 400 × 595, renamed `-TOOSMALL`.** What was probed:

- blodgett.com's WordPress media library exposes this asset in exactly two sizes,
  `CTB.jpg` (**400 × 595**, the original upload) and `CTB-202x300.jpg`. There is no
  `-scaled` variant, which means the upload was already under WordPress's 2560 px scaling
  threshold. `CTB-onStand.jpg`, `CTB-on-Stand.jpg` and `CTB-1.jpg` all 404 — the
  `-onStand` naming exists **only** for the CTBR (§8, the 2259 × 3503 file).
- Both the series page and the model page were scraped for `srcset`/`data-*`/`og:image`
  candidates; the only CTB references anywhere on blodgett.com are those same two sizes.
- The official spec PDF was opened with `fitz` and its embedded image objects extracted
  losslessly. Largest is **307 × 253** (the thumbnail render on page 1) — far worse.
- KaTom's Cloudinary CDN carries only three CTB assets, and they are the three 1600 × 1600
  shots already held; it has no on-stand composition. WebstaurantStore serves no CTB
  product image at all on that listing.

**400 × 595 is the ceiling for this particular render.** Anyone needing a large food-styled
on-stand image should use the 1600 × 1600 `-front` file plus the 1000 × 1000 `-alt` instead,
or commission the shot. ⚠ Do **not** substitute the 2259 × 3503 CTBR file to solve it — that
is the mirrored right-hinge sibling and is wrong for this SKU.
